...
async function loadProviders() {
    const response = await fetch('/api/workshop/providers?per_page=100', {
...
async function deleteProduct(product) {
    ...
    const response = await fetch(`/api/workshop/products/${product.id}`, {
        method: 'DELETE',
        headers: {
            Accept: 'application/json',
            'X-CSRF-TOKEN': csrfToken,
            ...
            <div class="overflow-hidden rounded-3xl border border-black/10">
                ...
                <div v-if="pagination" class="flex items-center justify-between border-t border-black/10 bg-[#faf8f2] px-5 py-4 text-sm">
                    ...
                    <div class="flex gap-2">
                        <button class="rounded-xl border border-black/10 px-3 py-2 disabled:opacity-40" :disabled="!pagination.prev_page_url" type="button" @click="loadProducts(pagination.prev_page_url)">Anterior</button>
                        <button class="rounded-xl border border-black/10 px-3 py-2 disabled:opacity-40" :disabled="!pagination.next_page_url" type="button" @click="loadProducts(pagination.next_page_url)">Proxima</button>
                    </div>
                </div>
            </div>
        </div>
